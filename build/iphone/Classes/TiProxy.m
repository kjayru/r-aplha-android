...
//Rare exceptions to indicate a bug in the _retoalpha code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_retoalpha = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_retoalpha]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.retoalpha.module"
